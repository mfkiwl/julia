        ...
        if head === :call
            ...
            if isa(f, IntrinsicFunction)
                ...
                return intrinsic_nothrow(f,
                        TypeLattice[argextype(ea[i], src, sptypes) for i = 2:length(ea)])
            end
            contains_is(_PURE_BUILTINS, f) && return true
            contains_is(_PURE_OR_ERROR_BUILTINS, f) || return false
            rt === ⊥ && return false
            return _builtin_nothrow(f, TypeLattice[argextype(ea[i], src, sptypes) for i = 2:length(ea)], rt)
        elseif head === :new
            a = ea[1]
...
argextype(@nospecialize(value), compact::IncrementalCompact, sptypes::Lattices) = compact_exprtype(compact, value)
